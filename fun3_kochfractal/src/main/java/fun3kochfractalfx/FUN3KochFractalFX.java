...
public class FUN3KochFractalFX extends Application{
    ...
    public void start(Stage primaryStage) {
        ...
        GridPane grid;
        grid = new GridPane();
        barBottom = new ProgressBar();
        barRight = new ProgressBar();
        barLeft = new ProgressBar();
        grid.add(barBottom, 1,10,1,1);
        grid.add(barRight, 2,10,1,1);
        grid.add(barLeft, 3,10,1,1);
        grid.setHgap(10);
        grid.setVgap(10);
    ...
    public  void showProgress(BottomCalculatable bottom, LeftCalculatable left, RightCalculatable right){
        barRight.progressProperty().bind(right.progressProperty());
        barBottom.progressProperty().bind(bottom.progressProperty());
        barLeft.progressProperty().bind(left.progressProperty());
    }
    ...
    private void increaseLevelButtonActionPerformed(ActionEvent event) {
        if (currentLevel < 12) {
            ...
            kochManager.changeLevel(currentLevel);
            showProgress(kochManager.bottomEdges, kochManager.leftEdges , kochManager.rightEdges);
